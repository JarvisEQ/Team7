...
class half_rainbow:
    ...
    def train(self, win_rate):
        ...
        # When calculating the loss, use clamp to clip it
        # this is how we get the proximal policy optimisation to work!
        loss = self.loss(current_qs, target)
        loss = torch.clamp(loss, min=-LOSS_MIN, max=LOSS_MAX)
        loss.backward()
        self.opti.step()
